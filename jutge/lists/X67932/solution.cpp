...
void Llista::fusiona_suma(Llista &l2, nat n) {
    node* pNode1 = _prim->seg;
    node* pNode2 = l2._prim->seg;

    // Eliminar el circular porque soy muy vago
    while (pNode1->seg != _prim) {
        pNode1 = pNode1->seg;
    }
    pNode1->seg = NULL;
    pNode1 = _prim->seg;

    while (pNode2->seg != l2._prim) {
        pNode2 = pNode2->seg;
    }
    pNode2->seg = NULL;
    pNode2 = l2._prim->seg;
    ...
    pNode1 = _prim;
    node* pNode1Ant = _prim;
    while (pNode1->seg != NULL) {
        pNode1 = pNode1->seg;
        pNode1->ant = pNode1Ant;
        pNode1Ant = pNode1Ant->seg;
    }
    pNode1->seg = _prim;
    _prim->ant = pNode1;
}